...
def validate():
    if request.method=='POST':
        ...
        if validate_words():
            ...
            return render_template('on_leaderboard.html', title="Enter your name!", time=session.get('stoptime'))
        else:
...
def add_to_leaderboard():
        if(session.get('running') == True):
            if request.method=='POST':
                ...
                elif float(session.get('stoptime') > float(longleaderboard[len(longleaderboard) - 1][1])):
                    longleaderboard.append(session.get('player_score'))
        ...
        return render_template('leaderboard.html', leaderboard = shortleaderboard, position=session.get('position'), empty=session.get('empty'))
